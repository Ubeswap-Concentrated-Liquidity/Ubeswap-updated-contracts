...
import { MockTimeUbeswapPool } from "../typechain/test/MockTimeUbeswapPool";
import { TickMathTest } from "../typechain/test/TickMathTest";
import { UbeswapPoolSwapTest } from "../typechain/test/UbeswapPoolSwapTest";
...
describe("UbeswapPool arbitrage tests", () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(
          ...
          const arbTestFixture = async () => {
            ...
            const tester = (await testerFactory.deploy()) as UbeswapPoolSwapTest;

            const tickMathFactory = await ethers.getContractFactory(
              "TickMathTest"
            );
          ...
          let tester: UbeswapPoolSwapTest;
          ...
          async function simulateSwap(
            ...
            const {
              ...
              amountSpecified,
              limitSqrtPrice ??
                (zeroToOne ? MIN_SQRT_RATIO.add(1) : MAX_SQRT_RATIO.sub(1))
            );
          ...
          for (const {
            ...
            describe(description, () => {
              function valueToken1(
                arbBalance0: BigNumber,
                arbBalance1: BigNumber
              ) {
              ...
              it("not sandwiched", async () => {
                const {
                  ...
                  amount1Delta,
                  amount0Delta,
                } = await simulateSwap(zeroToOne, inputAmount);
                zeroToOne
              ...
              it("sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity", async () => {
                ...
                const bottomTick = zeroToOne
                  ? tickAfterFirstTickAboveMarginPrice
                  : firstTickAboveMarginalPrice;
                const topTick = zeroToOne
                  ? firstTickAboveMarginalPrice
                  : tickAfterFirstTickAboveMarginPrice;
                ...
                expect({
                  ...
                  profit: {
                    final: formatTokenAmount(
                      valueToken1(arbBalance0, arbBalance1)
                    ),

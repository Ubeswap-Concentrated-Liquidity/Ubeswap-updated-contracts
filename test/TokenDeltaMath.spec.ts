...
describe("TokenDeltaMath", () => {
  ...
  describe("#getNewPriceAfterInput", () => {
    it("fails if price is zero", async () => {
      await expect(
        sqrtPriceMath.getNewPriceAfterInput(
          0,
          0,
  ...
  describe("#getNewPriceAfterOutput", () => {
    it("fails if price is zero", async () => {
      await expect(
        sqrtPriceMath.getNewPriceAfterOutput(
          0,
          0,
          expandTo18Decimals(1).div(10),
          false
    ...
    it("fails if output amount is exactly the virtual reserves of token0", async () => {
      ...
      const amountOut = 4;
      await expect(
        sqrtPriceMath.getNewPriceAfterOutput(price, liquidity, amountOut, false)
      ).to.be.reverted;
    });
    ...
    it("fails if output amount is greater than virtual reserves of token1", async () => {
      ...
      await expect(
        sqrtPriceMath.getNewPriceAfterOutput(price, liquidity, amountOut, true)
      ).to.be.reverted;
    });
    ...
    it("returns input price if amount in is zero and zeroToOne = true", async () => {
      const price = encodePriceSqrt(1, 1);
      expect(
        await sqrtPriceMath.getNewPriceAfterOutput(
          price,
          expandTo18Decimals(1).div(10),
          0,
          true
  ...
  describe("#getToken0Delta", () => {
    it("fails if prices are incorrect", async () => {
      ...
      await expect(
        sqrtPriceMath.getToken0Delta(0, encodePriceSqrt(2, 1), 0, true)
      ).to.be.reverted;
    });
  ...
  describe("#getToken1Delta", () => {
    ...
    it("returns 0 if prices are equal", async () => {
      const amount1 = await sqrtPriceMath.getToken0Delta(
        encodePriceSqrt(1, 1),
        encodePriceSqrt(1, 1),
        0,
        true
  ...
  describe("swap computation", () => {
    it("sqrtP * sqrtQ overflows", async () => {
      ...
      const amount0Delta = await sqrtPriceMath.getToken0Delta(
        sqrtQ,
        sqrtP,
        liquidity,
        true

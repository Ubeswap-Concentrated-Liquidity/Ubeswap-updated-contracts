...
import { TestUbeswapSwapPay } from "../typechain/TestUbeswapSwapPay";
...
import { TestUbeswapReentrantCallee } from "../typechain/TestUbeswapReentrantCallee";
import { TickMathTest } from "../typechain/TickMathTest";
import { PriceMovementMathTest } from "../typechain/PriceMovementMathTest";

type ThenArg<T> = T extends PromiseLike<infer U> ? U : T;

describe("UbeswapPool", () => {
  ...
  let factory: UbeswapFactory;
  let pool: MockTimeUbeswapPool;

  let swapTarget: TestUbeswapCallee;
    ...
    describe("after initialization", () => {
      ...
      describe("failure cases", () => {
        describe("underpayment", () => {
          let payer: TestUbeswapSwapPay;

          beforeEach(async () => {
            const factory = await ethers.getContractFactory(
              "TestUbeswapSwapPay"
            );
            payer = (await factory.deploy()) as TestUbeswapSwapPay;
          ...
          it("fails if token0 payed 0", async () => {
            ...
            ).to.be.revertedWith("IIAM");
            await expect(
              payer.mint(pool.address, wallet.address, -22980, 0, 10000, 0, 100)
            ).to.be.revertedWith("IIAM");
          });
      ...
      describe("success cases", () => {
        ...
        describe("above current price", () => {
          ...
          it("adds liquidity to liquidityTotal", async () => {
            ...
            expect((await pool.ticks(tickSpacing)).liquidityTotal).to.eq(150);
            expect((await pool.ticks(tickSpacing * 2)).liquidityTotal).to.eq(
              60
            );
          });
      ...
      it("community fees accumulate as expected during swap", async () => {
        ...
        expect((await token0.balanceOf(vaultAddress)).toString()).to.eq(
          "1700000000000"
        );
        expect((await token1.balanceOf(vaultAddress)).toString()).to.eq(
          "170000000000"
        );
      });
      ...
      it("positions are protected before community fee is turned on", async () => {
        ...
        expect(Number((await token0.balanceOf(vaultAddress)).toString())).to.eq(
          0
        );
        expect(Number((await token1.balanceOf(vaultAddress)).toString())).to.eq(
          0
        );
      ...
      it("poke is not allowed on uninitialized position", async () => {
        ...
        expect(innerFeeGrowth0Token).to.eq(
          "3402823669209373878308127703486852"
        );
  ...
  describe("miscellaneous mint tests", () => {
    ...
    it("cannot remove more than the entire position", async () => {
      ...
      await expect(pool.burn(bottomTick, topTick, expandTo18Decimals(1001))).to
        .be.reverted;
    });
    ...
    it("limit selling 0 for 1 at tick 0 thru 1", async () => {
      ...
      await expect(pool.collect(wallet.address, 0, 120, MaxUint128, MaxUint128))
        ...
        .withArgs(
          pool.address,
          wallet.address,
          BigNumber.from("6018336102428407")
      ...
      it("limit selling 0 for 1 at tick 0 thru 1", async () => {
        ...
        await expect(
          ...
          .withArgs(
            pool.address,
            wallet.address,
            BigNumber.from("6017734268818165").add("499521896501")
          );
        expect((await pool.globalState()).tick).to.be.gte(120);
      });
      it("limit selling 1 for 0 at tick 0 thru -1", async () => {
  ...
  describe("#collect", () => {
    ...
    describe("works across large increases", () => {
      ...
      it("works just before the cap binds", async () => {
        ...
        const { fees0, fees1 } = await pool.positions(
          await getPositionKey(wallet.address, minTick, maxTick, pool)
        );
  ...
  describe("#communityFee", () => {
    ...
    it("cannot be changed by addresses that are not owner", async () => {
      await expect(pool.connect(other).setCommunityFee(170, 170)).to.be
        .reverted;
    });
    ...
    it("fees collected by lp after two swaps with intermediate withdrawal", async () => {
      ...
      const {
        token0Fees: token0FeesNext,
        token1Fees: token1FeesNext,
      } = await swapAndGetFeesOwed({
        amount: expandTo18Decimals(1),
        zeroToOne: true,
        poke: false,
  ...
  it("tickMath handles tick overflow", async () => {
    ...
    await expect(sqrtTickMath.getSqrtRatioAtTick(887273)).to.be.revertedWith(
      "T"
    );
    await expect(sqrtTickMath.getSqrtRatioAtTick(-887273)).to.be.revertedWith(
      "T"
  ...
  xit("tick transition cannot run twice if zero for one swap ends at fractional price just below tick", async () => {
    ...
    expect((await pool.globalState()).tick, "pool tick is -24081").to.eq(
      -24081
    );
    ...
    // check the math works out to moving the price down 1, sending no amount out, and having some amount remaining
    {
      const {
        ...
        amountOut,
        sqrtQ,
      } = await PriceMovementMath.movePriceTowardsTarget(
        p0,
        p0.sub(1),
        liquidity,
        3,
  ...
  describe("#adaptiveFee", function () {
    ...
    async function getStatistics(time: number) {
      ...
      return [
        now.volatilityCumulatives[0]
          .sub(then.volatilityCumulatives[0])
          .div(BigNumber.from(DAY)),
        now.volumePerAvgLiquiditys[0].sub(then.volumePerAvgLiquiditys[0]),
        now.secondsPerLiquidityCumulatives[0].sub(
          then.secondsPerLiquidityCumulatives[0]
        ),
        time,
      ];
    }
    ...
    it("single huge step after day", async () => {
      ...
      await mint(
        ...
        -24000,
        24000,
        liquidity.mul(BigNumber.from(1000000000))
      );
      ...
      await swapExact1For0(
        liquidity.mul(BigNumber.from(AMOUNT)),
        wallet.address
      );
      await pool.advanceTime(60);
      ...
      for (let i = 0; i < 25; i++) {
        ...
        stats.push(
          `Fee: ${fee}, Avg_volat: ${avrges.TWVolatilityAverage.toString()}, Avg_Vol_per_liq: ${avrges.TWVolumePerLiqAverage.toString()} `
        );
    ...
    xit("changes", async () => {
      ...
      console.log(
        "Volm:",
        BigNumber.from(stats[1]).div(BigNumber.from(10).pow(18)).toString()
      );
      ...
      console.log(
        ...
        BigNumber.from(stats[1])
          ...
          .div(BigNumber.from(DAY).mul(BigNumber.from(2).pow(128)))
          .toString()
      );
      console.log(DAY);
    });
  ...
  describe("#flash", () => {
    ...
    describe("after liquidity added", () => {
      ...
      beforeEach("add some tokens", async () => {
        await initializeAtZeroTick(pool);
        [balanceToken0, balanceToken1] = await Promise.all([
      ...
      describe("fee off", () => {
        ...
        it("fails if flash amount is greater than token balance", async () => {
          await expect(
            flash(balanceToken0.add(1), balanceToken1, other.address)
          ).to.be.reverted;
          await expect(
            flash(balanceToken0, balanceToken1.add(1), other.address)
          ).to.be.reverted;
        });
      ...
      describe("fee on", () => {
        ...
        it("increases the fee growth by the expected amount", async () => {
          ...
          expect(
            Number((await token0.balanceOf(vaultAddress)).toString())
          ).to.eq(0);
          expect(
            Number((await token1.balanceOf(vaultAddress)).toString())
          ).to.eq(1);
        });
        it("allows donating token0", async () => {
          await expect(flash(0, 0, constants.AddressZero, 567, 0))
  ...
  describe("#setCommunityFee", () => {
    ...
    it("can only be called by factory owner", async () => {
      await expect(pool.connect(other).setCommunityFee(200, 200)).to.be
        .reverted;
    });
  ...
  describe("#lock", () => {
    ...
    it("cannot reenter from swap callback", async () => {
      const reentrant = (await (
        await ethers.getContractFactory("TestUbeswapReentrantCallee")
      ).deploy()) as TestUbeswapReentrantCallee;

      // the tests happen in solidity
      await expect(reentrant.swapToReenter(pool.address)).to.be.revertedWith(
        "Unable to reenter"
      );
    });
  });
  ...
  describe("#getInnerCumulatives", () => {
    ...
    it("throws if ticks are in reverse order", async () => {
      await expect(pool.getInnerCumulatives(topTick, bottomTick)).to.be
        .reverted;
    });
    ...
    it("throws if tick lower is too low", async () => {
      await expect(
        pool.getInnerCumulatives(getMinTick(tickSpacing) - 1, topTick)
      ).be.reverted;
    });
    it("throws if tick upper is too high", async () => {
      await expect(
        pool.getInnerCumulatives(bottomTick, getMaxTick(tickSpacing) + 1)
      ).be.reverted;
    });
    it("throws if tick lower is not initialized", async () => {
      await expect(pool.getInnerCumulatives(bottomTick - tickSpacing, topTick))
        .to.be.reverted;
    });
    it("throws if tick upper is not initialized", async () => {
      await expect(pool.getInnerCumulatives(bottomTick, topTick + tickSpacing))
        .to.be.reverted;
    });
    ...
    it("increases by expected amount when time elapses in the range", async () => {
      ...
      expect(innerSecondsSpentPerLiquidity).to.eq(
        BigNumber.from(5).shl(128).div(10)
      );
    ...
    it("time increase below range is not counted", async () => {
      ...
      expect(innerSecondsSpentPerLiquidity).to.eq(
        BigNumber.from(7).shl(128).div(10)
      );
    ...
    it("positions minted after time spent", async () => {
      ...
      expect(innerSecondsSpentPerLiquidity).to.eq(
        BigNumber.from(8).shl(128).div(15)
      );
    ...
    it("overlapping liquidity is aggregated", async () => {
      ...
      expect(innerTickCumulative, "innerTickCumulative").to.eq(0);
      expect(innerSecondsSpentPerLiquidity).to.eq(
        BigNumber.from(5).shl(128).div(25)
      );
      expect(innerSecondsSpent).to.eq(5);
    });
    it("relative behavior of snapshots", async () => {
      ...
      const expectedDiffSecondsPerLiquidity = BigNumber.from(3)
        .shl(128)
        .div(15);
      expect(
        innerSecondsSpentPerLiquidity.sub(innerSecondsSpentPerLiquidityStart)
      ).to.eq(expectedDiffSecondsPerLiquidity);
      expect(innerSecondsSpentPerLiquidity).to.not.eq(
      ...
      expect(
        innerTickCumulative.sub(innerTickCumulativeStart),
        "innerTickCumulative"
      ).to.eq(-20796);
  ...
  describe("swap underpayment tests", () => {
    let underpay: TestUbeswapSwapPay;
    beforeEach("deploy swap test", async () => {
      const underpayFactory = await ethers.getContractFactory(
        "TestUbeswapSwapPay"
      );
      underpay = (await underpayFactory.deploy()) as TestUbeswapSwapPay;
  ...
  describe("virtual pool tests", () => {
    ...
    beforeEach("deploy virtualPoolMock", async () => {
      ...
      const virtualPoolMockFactory = await ethers.getContractFactory(
        "MockTimeVirtualPool"
      );
    ...
    it("swap with finished incentive", async () => {
      ...
      expect(await pool.activeIncentive()).to.be.eq(

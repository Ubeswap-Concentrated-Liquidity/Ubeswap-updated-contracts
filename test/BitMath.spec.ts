...
describe("BitMath", () => {
  ...
  describe("#mostSignificantBit", () => {
    ...
    it("all powers of 2", async () => {
      const results = await Promise.all(
        [...Array(255)].map((_, i) =>
          bitMath.mostSignificantBit(BigNumber.from(2).pow(i))
        )
      );
    ...
    it("uint256(-1)", async () => {
      expect(
        await bitMath.mostSignificantBit(BigNumber.from(2).pow(256).sub(1))
      ).to.eq(255);
    });

    it("gas cost of smaller number  [ @skip-on-coverage ]", async () => {
      await snapshotGasCost(
        bitMath.getGasCostOfMostSignificantBit(BigNumber.from(3568))
      );
    });
    it("gas cost of min number  [ @skip-on-coverage ]", async () => {
      await snapshotGasCost(
        bitMath.getGasCostOfMostSignificantBit(BigNumber.from(1))
      );
    });
  ...
  describe("#leastSignificantBit", () => {
    ...
    it("all powers of 2", async () => {
      const results = await Promise.all(
        [...Array(255)].map((_, i) =>
          bitMath.leastSignificantBit(BigNumber.from(2).pow(i))
        )
      );
      expect(results).to.deep.eq([...Array(255)].map((_, i) => i));
    });
    it("uint256(-1)", async () => {
      expect(
        await bitMath.leastSignificantBit(BigNumber.from(2).pow(256).sub(1))
      ).to.eq(0);
    });

    it("gas cost of smaller number  [ @skip-on-coverage ]", async () => {
      await snapshotGasCost(
        bitMath.getGasCostOfLeastSignificantBit(BigNumber.from(3568))
      );
    });
    it("gas cost of min number  [ @skip-on-coverage ]", async () => {
      await snapshotGasCost(
        bitMath.getGasCostOfLeastSignificantBit(BigNumber.from(1))
      );
    });
    it("gas cost of max uint128  [ @skip-on-coverage ]", async () => {
      await snapshotGasCost(
        bitMath.getGasCostOfLeastSignificantBit(
          BigNumber.from(2).pow(128).sub(1)
        )
      );
    });

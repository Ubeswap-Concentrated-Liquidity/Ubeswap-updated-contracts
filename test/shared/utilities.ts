...
export function createPoolFunctions({
  ...
  const swapExact0For1SupportingFee: SwapFunction = (
    amount,
    to,
    limitSqrtPrice
  ) => {
  ...
  const swapExact1For0SupportingFee: SwapFunction = (
    amount,
    to,
    limitSqrtPrice
  ) => {
...
export function createMultiPoolFunctions({
  ...
  swapTarget: TestUbeswapRouter;
  poolInput: MockTimeUbeswapPool;
  poolOutput: MockTimeUbeswapPool;
}): MultiPoolFunctions {

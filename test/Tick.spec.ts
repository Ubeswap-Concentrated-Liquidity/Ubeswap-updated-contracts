...
describe("Tick", () => {
  ...
  describe("#getInnerFeeGrowth", () => {
    it("returns all for two uninitialized ticks if tick is inside", async () => {
      ...
      } = await tickTest.getInnerFeeGrowth(-2, 2, 0, 15, 15);
    ...
    it("returns 0 for two uninitialized ticks if tick is above", async () => {
      const {
        innerFeeGrowth0Token,
        innerFeeGrowth1Token,
      } = await tickTest.getInnerFeeGrowth(-2, 2, 4, 15, 15);
  ...
  describe("#update", async () => {
    it("flips from zero to nonzero", async () => {
      expect(
        await tickTest.callStatic.update(0, 0, 1, 0, 0, 0, 0, 0, false)
      ).to.eq(true);
    });
    ...
    it("reverts if total liquidity gross is greater than max", async () => {
      await tickTest.update(
        0,
        0,
        BigNumber.from("11505743598341114571880798222544994").div(2),
        0,
        0,
        0,
        0,
    ...
    it("reverts on overflow liquidity gross", async () => {
      await tickTest.update(
        0,
        0,
        BigNumber.from("11505743598341114571880798222544994").div(2).sub(1),
        0,
        0,
        0,
        0,
      ...
      await expect(
        tickTest.update(0, 0, MaxUint128.div(2).sub(1), 0, 0, 0, 0, 0, false)
      ).to.be.reverted;
    });

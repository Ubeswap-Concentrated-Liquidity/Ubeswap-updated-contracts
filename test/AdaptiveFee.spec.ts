...
describe("AdaptiveFee", () => {
  ...
  describe("#getFee", () => {
    it("fee: 0 volat 0 volume", async () => {
      console.log(
        (
          await adaptiveFee.getFee(BigNumber.from(0), BigNumber.from(0))
        ).toString()
      );
    });
    ...
    it("fee grid snapshot", async () => {
      ...
      const getFee = (volume: any, volatility: any) => {
        let sigm1 = 0;
        if (config.beta1 - volatility <= -6 * config.gamma1)
          sigm1 = config.alpha1;
        else if (config.beta1 - volatility >= 6 * config.gamma1) sigm1 = 0;
        else
          sigm1 =
            config.alpha1 /
            (1 + Math.exp((config.beta1 - volatility) / config.gamma1));

        let sigm2 = 0;
        if (config.beta2 - volatility <= -6 * config.gamma2)
          sigm2 = config.alpha2;
        else if (config.beta2 - volatility >= 6 * config.gamma2) sigm2 = 0;
        else
          sigm2 =
            config.alpha2 /
            (1 + Math.exp((config.beta2 - volatility) / config.gamma2));

        let volumeSigm = 0;
        if (config.volumeBeta - volume <= -6 * config.volumeGamma)
          volumeSigm = sigm1 + sigm2;
        else if (config.volumeBeta - volume >= 6 * config.volumeGamma)
          volumeSigm = 0;
        else
          volumeSigm =
            (sigm1 + sigm2) /
            (1 + Math.exp((config.volumeBeta - volume) / config.volumeGamma));
      ...
      for (let vol of volumes) {
        ...
        for (let volat of volats) {
          ...
          let cFee = Number(
            (
              await adaptiveFee.getFee(
                BigNumber.from(volat),
                BigNumber.from(vol)
              )
            ).toString()
          );
  ...
  describe("#getFee gas cost  [ @skip-on-coverage ]", () => {
    it("gas cost of 0 volat 0 volume", async () => {
      await snapshotGasCost(
        adaptiveFee.getGasCostOfGetFee(BigNumber.from(0), BigNumber.from(0))
      );
    });

    it("gas cost of 0 volat 100 volume", async () => {
      await snapshotGasCost(
        adaptiveFee.getGasCostOfGetFee(BigNumber.from(0), BigNumber.from(100))
      );
    });

    it("gas cost of 100 volat 0 volume", async () => {
      await snapshotGasCost(
        adaptiveFee.getGasCostOfGetFee(BigNumber.from(100), BigNumber.from(0))
      );
    });

    it("gas cost of 100 volat 100 volume", async () => {
      await snapshotGasCost(
        adaptiveFee.getGasCostOfGetFee(BigNumber.from(100), BigNumber.from(100))
      );
    });

    it("gas cost of 2000 volat 100 volume", async () => {
      await snapshotGasCost(
        adaptiveFee.getGasCostOfGetFee(
          BigNumber.from(2000),
          BigNumber.from(100)
        )
      );

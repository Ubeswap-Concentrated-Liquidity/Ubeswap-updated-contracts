...
import { TickTableTest } from "../typechain/TickTableTest";
...
describe("TickTable", () => {
  ...
  beforeEach("deploy TickTableTest", async () => {
    const tickTableTestFactory = await ethers.getContractFactory(
      "TickTableTest"
    );
  ...
  describe("#nextTickInTheSameRow", () => {
    ...
    describe("lte = false", async () => {
      it("returns tick to right if at initialized tick", async () => {
        const { next, initialized } = await tickTable.nextTickInTheSameRow(
          78 * 60,
          false
        );
      ...
      it("gas cost on boundary  [ @skip-on-coverage ]", async () => {
        await snapshotGasCost(
          await tickTable.getGasCostOfNextTickInTheSameRow(255 * 60, false)
        );
      });
      it("gas cost just below boundary  [ @skip-on-coverage ]", async () => {
        await snapshotGasCost(
          await tickTable.getGasCostOfNextTickInTheSameRow(254 * 60, false)
        );
      });
      it("gas cost for entire word  [ @skip-on-coverage ]", async () => {
        await snapshotGasCost(
          await tickTable.getGasCostOfNextTickInTheSameRow(768 * 60, false)
        );
      });
    });
    ...
    describe("lte = true", () => {
      ...
      it("gas cost on boundary  [ @skip-on-coverage ]", async () => {
        await snapshotGasCost(
          await tickTable.getGasCostOfNextTickInTheSameRow(256 * 60, true)
        );
      });
      it("gas cost just below boundary  [ @skip-on-coverage ]", async () => {
        await snapshotGasCost(
          await tickTable.getGasCostOfNextTickInTheSameRow(255 * 60, true)
        );
      });
      it("gas cost for entire word  [ @skip-on-coverage ]", async () => {
        await snapshotGasCost(
          await tickTable.getGasCostOfNextTickInTheSameRow(1024 * 60, true)
        );
      });
    });
  });

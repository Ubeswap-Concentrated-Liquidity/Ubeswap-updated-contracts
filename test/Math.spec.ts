...
describe("SafeMath", () => {
  ...
  it("#add", async () => {
    await expect(safeMath.add(BigNumber.from(2).pow(256).sub(1), 1)).to.be
      .reverted;
  });
  ...
  it("#addInt", async () => {
    await expect(safeMath.addInt(BigNumber.from(2).pow(255).sub(1), 1)).to.be
      .reverted;
  });

  it("#subInt", async () => {
    await expect(
      safeMath.subInt(100, BigNumber.from(2).pow(255).sub(1).mul(-1))
    ).to.be.reverted;
    await expect(safeMath.subInt(BigNumber.from(2).pow(255).sub(1), -100)).to.be
      .reverted;
  });
  ...
  it("#toUint160", async () => {
    await expect(safeMath.toUint160(BigNumber.from(2).pow(255).sub(1))).to.be
      .reverted;
  });

  it("#toInt128", async () => {
    await expect(safeMath.toInt128(BigNumber.from(2).pow(255).sub(1))).to.be
      .reverted;
  });

  it("#toInt256", async () => {
    await expect(safeMath.toInt256(BigNumber.from(2).pow(256).sub(1))).to.be
      .reverted;
  });
});
...
describe("FullMath", () => {
  ...
  describe("#mulDiv", () => {
    ...
    it("reverts on overflow with all max inputs", async () => {
      await expect(fullMath.mulDiv(MaxUint256, MaxUint256, MaxUint256.sub(1)))
        .to.be.reverted;
    });
  ...
  describe("#mulDivRoundingUp", () => {
    ...
    it("reverts on overflow with all max inputs", async () => {
      await expect(
        fullMath.mulDivRoundingUp(MaxUint256, MaxUint256, MaxUint256.sub(1))
      ).to.be.reverted;
    });
  ...
  function pseudoRandomBigNumber() {
    return BigNumber.from(
      new Decimal(MaxUint256.toString())

...
contract UbeswapPool is PoolState, PoolImmutables, IUbeswapPool {
  ...
  modifier onlyFactoryOwner() {
    require(msg.sender == IUbeswapFactory(factory).owner());
    _;
  }
  ...
  /// @inheritdoc IUbeswapPoolDerivedState
  ...
  /// @inheritdoc IUbeswapPoolActions
    ...
    )
  {
    ...
    uint256 receivedAmount1;
    {
      ...
      IUbeswapMintCallback(msg.sender).ubeswapMintCallback(amount0, amount1, data);
  ...
  function _payCommunityFee(address token, uint256 amount) private {
    address vault = IUbeswapFactory(factory).vaultAddress();
  ...
  function _swapCallback(
    ...
    IUbeswapSwapCallback(msg.sender).ubeswapSwapCallback(amount0, amount1, data);
  }
  ...
  struct SwapCalculationCache {
    ...
    IUbeswapVirtualPool.Status incentiveStatus; // If there is an active incentive at the moment
    ...
    )
  {
    ...
    SwapCalculationCache memory cache;
    {
      ...
      if (activeIncentive != address(0)) {
        IUbeswapVirtualPool.Status _status = IUbeswapVirtualPool(activeIncentive).increaseCumulative(blockTimestamp);
        if (_status == IUbeswapVirtualPool.Status.NOT_EXIST) {
          activeIncentive = address(0);
        } else if (_status == IUbeswapVirtualPool.Status.ACTIVE) {
          cache.incentiveStatus = IUbeswapVirtualPool.Status.ACTIVE;
        } else if (_status == IUbeswapVirtualPool.Status.NOT_STARTED) {
          cache.incentiveStatus = IUbeswapVirtualPool.Status.NOT_STARTED;
        }
      }
    ...
    while (true) {
      ...
      if (currentPrice == step.nextTickPrice) {
        ...
        if (step.initialized) {
          ...
          if (cache.incentiveStatus != IUbeswapVirtualPool.Status.NOT_EXIST) {
            IUbeswapVirtualPool(activeIncentive).cross(step.nextTick, zeroToOne);
          }
          int128 liquidityDelta;
          if (zeroToOne) {
  ...
  function flash(
    ...
    IUbeswapFlashCallback(msg.sender).ubeswapFlashCallback(fee0, fee1, data);
  ...
  /// @inheritdoc IUbeswapPoolPermissionedActions
  ...
  function setIncentive(address virtualPoolAddress) external override {
    require(msg.sender == IUbeswapFactory(factory).farmingAddress());

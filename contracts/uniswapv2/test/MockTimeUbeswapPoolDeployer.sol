...
import './MockTimeUbeswapPool.sol';
import '../DataStorageOperator.sol';

contract MockTimeUbeswapPoolDeployer {
  struct Parameters {
    address dataStorage;
    address factory;
    address token0;
  ...
  function deployMock(
    ...
    bytes32 initCodeHash = keccak256(type(MockTimeUbeswapPool).creationCode);
    ...
    pool = address(new MockTimeUbeswapPool{salt: keccak256(abi.encode(token0, token1))}());

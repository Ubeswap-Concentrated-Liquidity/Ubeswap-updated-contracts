...
import './SimulationTimeUbeswapPool.sol';

contract SimulationTimePoolDeployer is IUbeswapPoolDeployer {
  struct Parameters {
    address dataStorage;
    address factory;
    address token0;
  ...
  function deploy(
    ...
    pool = address(new SimulationTimeUbeswapPool{salt: keccak256(abi.encode(token0, token1))}());
  }
}

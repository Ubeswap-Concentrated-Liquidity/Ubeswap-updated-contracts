...
import { SimulationTimeUbeswapPool } from "../typechain/SimulationTimeUbeswapPool";
...
const FEE_CONFIGURATION = {
  // can be changed for different fee behavior
  alpha1: 2900,
  alpha2: 15000 - 3000,
  beta1: 360,
  beta2: 60000,
...
let createPool = async (
  ...
  swapTarget = (await calleeContractFactory.deploy()) as TestUbeswapCallee;

  const tx = await factory.createPool(firstToken.address, secondToken.address);

  const receipt = await tx.wait();
  const poolAddress = receipt.events?.[1].args?.pool as string;
  console.log("Pool: ", poolAddress);
  return poolFactory.attach(poolAddress) as SimulationTimeUbeswapPool;
};
...
async function main() {
  ...
  const blocks = JSON.parse(
    fs
      .readFileSync(path.resolve(__dirname, "AllBlocks_timestamped.json"))
      .toString()
  );
  let fees: any[] = [];
  let volats: any[] = [];
  ...
  ({
    ...
    token1,
    swapTarget,
    pool: pool as MockTimeUbeswapPool,
  }));
  ...
  for (let blockNum = 0; blockNum < numOfBlocks; blockNum++) {
    ...
    if (lastTimestamp == 0) {
      ...
      if (lastTimestamp !== block[0].timestamp) {
        await pool.advanceTime(
          BigNumber.from(block[0].timestamp).sub(BigNumber.from(lastTimestamp)),
          { nonce, maxFeePerGas: GAS_PRICE, maxPriorityFeePerGas: GAS_PRICE }
        );
        nonce++;
        lastTimestamp = block[0].timestamp;
      }
    ...
    for (let evNum = 0; evNum < block.length; evNum++) {
      ...
      switch (event.event) {
        ...
        case "Burn":
          ...
          await pool.burn(values.bottomTick, values.topTick, values.amount, {
            nonce,
            maxFeePerGas: GAS_PRICE,
            maxPriorityFeePerGas: GAS_PRICE,
          });
        ...
        case "Swap":
          ...
          if (ZtO) {
            ...
            if (values.amount1 < 0)
              console.log("ERR: SWAP 1 -> 0", values.amount1, values.amount0);
            if (values.amount1 == 0) {
              console.error(
                "INVALID INPUT DATA: SWAP 1 -> 0, ZERO INPUT AMOUNT"
              );
              continue;
            }
          }
          ...
          if (ZtO) {
            if (SIMULATE_VIA_PRICES)
              await swapTarget.swapToLowerSqrtPrice(
                pool.address,
                values.price,
                wallet.address,
                {
            ...
            if (SIMULATE_VIA_PRICES)
              await swapTarget.swapToHigherSqrtPrice(
                pool.address,
                values.price,
                wallet.address,
                {
    ...
    try {
      ...
      stats = [
        now.volatilityCumulatives[0].div(BigNumber.from(DAY)),
    ...
    if (packNumber !== currentPack) {
      ...
      fs.writeFileSync(
        path.resolve(__dirname, `results_${currentPack}.json`),
        JSON.stringify(res)
      );
    ...
    printProgress(
      (100 * (blockNum / numOfBlocks)).toFixed(2),
      timeConverter(block[0].timestamp),
      fee,
      stats[0].toString()
    );
  }
...
async function getStatistics(time: number) {
  let points = await pool.getTimepoints([
    BigNumber.from(0),
    BigNumber.from(time),
  ]);
  return [
    points.volatilityCumulatives[0]
      .sub(points.volatilityCumulatives[1])
      .div(BigNumber.from(DAY))
      .toString(),
    points.volumePerAvgLiquiditys[0]
      .sub(points.volumePerAvgLiquiditys[1])
      .toString(),
    points.secondsPerLiquidityCumulatives[0]
      .sub(points.secondsPerLiquidityCumulatives[1])
      .toString(),
    time,
  ];
}
